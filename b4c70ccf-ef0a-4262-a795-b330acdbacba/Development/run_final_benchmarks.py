...
print(f"Running final_bench [sample_size={SAMPLE_SIZE}, warm_up={WARM_UP_TIME}s] …")
rc = os.system(
    f'({BASE_ENV} '
...
if not _success:
    print("\n[Build/run FAILED — last 120 lines:]")
    for _l in _lines[-120:]:
        print(_l)
...
SIZE_ORDER = [1024, 8192, 65536, 256*1024, 1024*1024, 8*1024*1024]
...
GROUP_ORDER = ["buffer_pool", "arena_fixed", "datapath_with_refill"]

for _grp in GROUP_ORDER:
    rows = _groups.get(_grp, [])
    if not rows:
        continue
    print(f"\n{'─'*110}")
    print(f"  GROUP: {_grp}")
    print(f"  {'Size/Count':<12} {'Variant':<35} {'Time (mid)':<22} {'Throughput (mid)'}")
    print(f"  {'─'*10} {'─'*33} {'─'*20} {'─'*25}")
    rows.sort(key=lambda r: (SIZE_ORDER.index(r[0]) if r[0] in SIZE_ORDER else r[0], r[1]))
    for sz, var, time_str, thrpt_str in rows:
        ...
        label = _fmt_size(sz) if sz >= 1024 else f"{sz} B"
        print(f"  {label:<12} {var:<35} {_t:<22} {_p}")
