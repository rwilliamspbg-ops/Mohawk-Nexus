...
# ── Step 3: Run the recommendations benchmark ─────────────────────────────────
SAMPLE_SIZE  = 300
...
if not _success:
    print("\n[Build/run FAILED — last 100 lines:]")
    for _l in _lines[-100:]:
        print(_l)
...
_GROUP_ORDER = ["fast_buffer_zero", "fast_buffer_0xAB", "bump_arena_hotpath", "conditional_strategy"]

for _grp_name in _GROUP_ORDER:
    rows = _groups.get(_grp_name, [])
    if not rows:
        continue
    print(f"\n{'─'*105}")
    print(f"  GROUP: {_grp_name}")
    print(f"  {'Size':<10} {'Variant':<35} {'Time (lo / mid / hi)':<40} {'Throughput (mid)'}")
    print(f"  {'─'*8} {'─'*33} {'─'*38} {'─'*25}")
    ...
    for sz, var, time_str, thrpt_str in rows:
        ...
        _t = f"{mv:.3f} {mu}" if mv is not None else time_str[:35]
        _p = f"{pv:.2f} {pu}" if pv is not None else (thrpt_str or "")
        print(f"  {_fmt_size(sz):<10} {var:<35} {_t:<40} {_p}")
...
recommendations_bench_results = _results
