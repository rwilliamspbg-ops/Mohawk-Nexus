...
if not _ok:
    ...
    for l in _lines[-150:]:
        print(l)
    raise RuntimeError("zero_copy_bench failed — see output above")
...
GROUP_ORDER = ["zero_copy_e2e", "poll_cost_only", "forwarder_only"]
...
for grp in GROUP_ORDER:
    ...
    # collect baseline times per count for speedup calculation
    _baseline = {}
    for cnt, var, ts, _ in rows:
        if var == "baseline_clone" or var == "clone_templates" or var == "process_batch":
            mv, _ = _mid(ts)
            if mv:
                _baseline[cnt] = mv
    ...
    print(f"\n{'─'*105}")
    print(f"  GROUP: {grp}")
    print(f"  {'Count':<8} {'Variant':<30} {'Time (mid)':<22} {'Throughput (mid)':<25} {'vs baseline'}")
    print(f"  {'─'*6} {'─'*28} {'─'*20} {'─'*23} {'─'*12}")
    ...
    for cnt, var, time_str, thrpt_str in rows:
        ...
        if base and mv and var not in ("baseline_clone","clone_templates","process_batch"):
            ratio = base / mv
            _speedup = f"{ratio:.2f}×" if ratio > 1 else f"{1/ratio:.2f}× slower"

        print(f"  {cnt:<8} {var:<30} {_t:<22} {_p:<25} {_speedup}")
...
zero_copy_bench_results = _results
